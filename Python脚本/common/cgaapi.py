...
class AsyncWaitNotify:
    def __init__(self,global_wait_list,wait_mutex):
        self._async_wait_queue=queue.Queue()    
        self._global_wait_list = global_wait_list
        self._wait_mutex = wait_mutex
        self._wait_mutex.acquire()
        self._global_wait_list.append(self)
        self._wait_mutex.release()
...
TRADE_STUFFS_ITEM = 1
TRADE_STUFFS_PET = 2
TRADE_STUFFS_PETSKILL = 3
TRADE_STUFFS_GOLD = 4

TRADE_STATE_CANCEL = 0
TRADE_STATE_READY = 1
TRADE_STATE_CONFIRM = 2
TRADE_STATE_SUCCEED = 3
...
class CGAPI(CGAPython.CGA):   
   ...
   g_tradeStuffs_caches=[]                #交易物品缓存

   g_chatMsg_mutex= threading.Lock()             #聊天回调
   g_npcDialog_mutex= threading.Lock()           #对话框回调
   g_serverShutdonw_mutex= threading.Lock()      #游戏窗口关闭通知
   g_gameWndKeyDown_mutex= threading.Lock()      #游戏窗口按键通知
   g_battleAction_mutex= threading.Lock()        #战斗状态变化通知
   g_playerMenu_mutex= threading.Lock()          #菜单选择通知
   g_workingResult_mutex= threading.Lock()       #工作状态通知
   g_tradeStuffs_mutex= threading.Lock()         #交易物品变更
   g_tradeDialog_mutex= threading.Lock()         #交易对话框通知
   g_tradeState_mutex= threading.Lock()          #交易状态变更
   g_downMap_mutex= threading.Lock()             #下载地图通知
   g_connectionState_mutex= threading.Lock()     #连接状态变更
   g_unitMenu_mutex= threading.Lock()            #菜单项通知
   g_mqtt_msg_mutex= threading.Lock()            #mqtt订阅事件发布通知
   ...
   def init_log(self):
      ...
      if sLogState == "true" or sLogState == "1":
         sFileName=self.GetPlayerInfo().name
         sFileName=re.sub(r"[\\/:\*\?\"<>\|]", "_", sFileName)
         logPath=self.g_fz_Path+f"//log//{sFileName}.log"
   ...
   #合成制造-技能名称，物品名称，超时时间
   def Work(self,name,itemName="",timeout=6500):
      find_skill = self.FindPlayerSkill(name)
      if find_skill == None:
         self.log("没有【"+name+"】技能")
         return None
      subIndex = find_skill.lv
      self.SetWorkDelay(timeout)
      self.StartWork(find_skill.index,0)
      

   #鉴定、装饰物品
   # itempos : 操作的物品位置
   # immediate : 是否立即完成（高速鉴定）
   def ManipulateItemEx(self,name,itempos,immediate):
      skill = self.FindPlayerSkill(name)
      if skill == None:
         self.log("没有【"+name+"】技能")
         return False
      self.debug_log(f"name:{name},itempos:{itempos},immediate:{immediate}")
      self.SetImmediateDoneWork(immediate)
      self.StartWork(skill.index,0)
      if not self.AssessItem(skill.index,itempos):
         self.log("无法操作该物品")
         return False
      beginTime = time.time()      
     
      while True:
         res = self.WaitRecvWorkingResult(1)
         if res:
            return res
         #self.debug_log("未返回工作结果，重试")
         #重试一次
         craftStatus = self.GetCraftStatus()
         curTime=time.time()
         #self.debug_log(f"craftStatus:{craftStatus}")
         if(craftStatus == 0 or craftStatus == 2):            
            return self.ManipulateItemEx(name,itempos,immediate)        
         isImmediate = self.GetImmediateDoneWorkState()
         #self.debug_log(f"isImmediate:{isImmediate}")
         if(isImmediate != 2 and curTime > (beginTime + 120)):
            self.debug_log(f"isImmediate:{isImmediate} curTime>120s")
            return self.ManipulateItemEx(name,itempos,immediate)
         elif isImmediate == 2 and curTime > (beginTime +  5):
            self.debug_log(f"isImmediate:{isImmediate} curTime>5s")
            return self.ManipulateItemEx(name,itempos,immediate)     
      self.debug_log(f"ManipulateItemEx return None")
      return None

   #获取第一个未鉴定物品
   def FindAssessableItem(self):
      skill = self.FindPlayerSkill('鉴定')
      mp = self.GetPlayerInfo().mp
      items = self.GetBagItems()
      for item in items:
         if not item.assessed and item.level <= skill.lv and mp >= item.level * 10:
            return item	
      return None	

   #鉴定 递归调用
   def AssessAllItems(self):
      name ="鉴定"
      skill = self.FindPlayerSkill(name)
      if skill == None:
         self.log("没有【"+name+"】技能")
         return False
      item = self.FindAssessableItem()
      if(item):		
         self.ManipulateItemEx('鉴定',item.pos,True)
         self.setTimeout(self.AssessAllItems, 100)
      else:			
         return      
                  ...
                  if dwLastTime == 0:                     
                     ...
                     if dwCurTime -dwLastTime   > 10:
                           ...
                           if len(findPath) == 1 or  not isLoop:
                              ...
                              if self.AutoNavigator(findPath,False, False) == False:
                                 return False
                              dwLastTime=dwCurTime                            
                           else:
   ...
   def TradeStuffsNotify(self,val):
      ...
      self.g_tradeStuffs_mutex.acquire()
      self.TradeStuffsDoAsyncCall(val)
      self.g_tradeStuffs_mutex.release()
      self.debug_log(f"recv trade stuffs:fini ")
   
   def TradeStuffsDoAsyncCall(self,val):
      self.debug_log(f"TradeStuffsDoAsyncCall:")
      if len(self.g_tradeStuffs_asyncs) > 0:
         self.debug_log(f"g_tradeStuffs_asyncs > 0")
         for q in self.g_tradeStuffs_asyncs:
            q.put_chat_msg(val)
         self.g_tradeStuffs_asyncs.clear()
      else:   
         now_time = time.time()
         self.debug_log(f"TradeStuffsDoAsyncCall cache data")
         for v in self.g_tradeStuffs_caches[:]:
            if (now_time-v[0]) > 3:
               self.g_tradeStuffs_caches.remove(v)
         self.g_tradeStuffs_caches.append([now_time,val])
   ...
   def WaitRecvMqttMsg(self,tSecond=10):        
      testWait = AsyncWaitNotify(self.g_mqtt_msg_asyncs,self.g_mqtt_msg_mutex)       
      return testWait.wait_msg_timeout(tSecond)
   
   #API
   def WaitRecvChatMsg(self,tSecond=10):        
      testWait = AsyncWaitNotify(self.g_chatMsg_asyncs,self.g_chatMsg_mutex)      
      return testWait.wait_msg_timeout(tSecond)
   #API
   def WaitRecvNpcDialog(self,tSecond=10):        
      testWait = AsyncWaitNotify(self.g_npcDialog_asyncs, self.g_npcDialog_mutex)
      return testWait.wait_msg_timeout(tSecond)
   #API
   def WaitRecvWorkingResult(self,tSecond=10):        
      testWait = AsyncWaitNotify(self.g_workingResult_asyncs,self.g_workingResult_mutex)       
      return testWait.wait_msg_timeout(tSecond)
   #API
   def WaitRecvTradeDialog(self,tSecond=10):        
      testWait = AsyncWaitNotify(self.g_tradeDialog_asyncs,self.g_tradeDialog_mutex)       
      return testWait.wait_msg_timeout(tSecond)
   #API
   def WaitRecvTradeStuffs(self,tSecond=10):                 
      testWait = AsyncWaitNotify(self.g_tradeStuffs_asyncs,self.g_tradeStuffs_mutex)       
      now_time=time.time()
      if len(self.g_tradeStuffs_caches) > 0:
         self.debug_log(f"WaitRecvTradeStuffs TradeStuffsDoAsyncCall > 0")
         self.g_tradeStuffs_mutex.acquire()   
         for v in self.g_tradeStuffs_caches[:]:
            if (now_time - v[0]) > 3:
               self.g_tradeStuffs_caches.remove(v)
            else:
               self.TradeStuffsDoAsyncCall(v[1])
               self.g_tradeStuffs_caches.remove(v)
               break
         self.g_tradeStuffs_mutex.release()          
      return testWait.wait_msg_timeout(tSecond)
   ...
   def WaitRecvTradeState(self,tSecond=10):        
      testWait = AsyncWaitNotify(self.g_tradeState_asyncs,self.g_tradeState_mutex)       
      return testWait.wait_msg_timeout(tSecond)
   #API
   def WaitRecvPlayerMenu(self,tSecond=10):        
      testWait = AsyncWaitNotify(self.g_playerMenu_asyncs,self.g_playerMenu_mutex)       
      return testWait.wait_msg_timeout(tSecond)
   #API
   def WaitRecvUnitMenu(self,tSecond=10):        
      testWait = AsyncWaitNotify(self.g_unitMenu_asyncs,self.g_unitMenu_mutex)       
      return testWait.wait_msg_timeout(tSecond)
   #API
   def WaitRecvBattleAction(self,tSecond=10):        
      testWait = AsyncWaitNotify(self.g_battleAction_asyncs,self.g_battleAction_mutex)       
      return testWait.wait_msg_timeout(tSecond)
   #API
   def WaitRecvConnectionState(self,tSecond=10):        
      testWait = AsyncWaitNotify(self.g_connectionState_asyncs,self.g_connectionState_mutex)       
      return testWait.wait_msg_timeout(tSecond)
   #API
   def WaitRecvGameWndKeyDown(self,tSecond=10):        
      testWait = AsyncWaitNotify(self.g_gameWndKeyDown_asyncs,self.g_gameWndKeyDown_mutex)       
      return testWait.wait_msg_timeout(tSecond)
   #API
   def WaitRecvDownloadMap(self,tSecond=10):        
      testWait = AsyncWaitNotify(self.g_downMap_asyncs,self.g_downMap_mutex)       
      return testWait.wait_msg_timeout(tSecond)
   #API
   def WaitRecvServerShutdown(self,tSecond=10):        
      testWait = AsyncWaitNotify(self.g_serverShutdonw_asyncs,self.g_serverShutdonw_mutex)       
      return testWait.wait_msg_timeout(tSecond)
   ...
   def AutoEncounterEnemyThread(self):
      ...
      if (curPoint.x == startPoint.x and curPoint.y == startPoint.y):
         ...
         if (self.IsInNormalState()):
               self.ForceMoveTo(startPoint.x, startPoint.y, True)	
         self.debug_log("结束自动遇敌,起点不一致，移动到起点，返回")
   ...
   def SellPilePos(self,x,y,saleItem,count=20,maxCount=40):
	...
	#卖堆叠物
   def SellPileDir(self,dir,saleItem,count=20,maxCount=40):     
      loopCount=0      
      while loopCount < 2:         
         apiSaleItems=[]
         bagItems = self.GetItemsInfo()
         for v in bagItems:
            if(v.pos > 7 and v.name == saleItem and v.count>=count):
               ...
               apiSaleItem.itemid=v.itemid
               apiSaleItem.itempos=v.pos
               apiSaleItem.count=int(v.count/count)
               apiSaleItems.append(apiSaleItem)	         
         self.TurnAbout(dir)
         self.WaitRecvNpcDialog()		
         self.ClickNPCDialog(-1,0)#	--这边没有类型判断 直接-1了
   ...
   def GetBagItems(self):
      ...
      itemsInfo = self.GetItemsInfo()
      for item in itemsInfo:
         if item.pos >= 8 and item.pos < 100:
   ...
   #扔叠加物
   def ThrowNotFullItemName(self,name,itemCount):
      itemsInfo=self.GetItemsInfo()     
      for itemInfo in itemsInfo:
         if itemInfo.name == name and itemInfo.count < itemCount:
            self.DropItem(itemInfo.pos)
 ...
 #     return self.TradeInternal(sName, None, None)
   def TradeInternal(self,myTradeStuffs,tgtTradeStuffs):
      ...
      self.WaitTradeMsg()
   def WaitTradeStuffs(self):
      recv_stuffs={"items":[],"pets":{},"petskills":{}}
      is_recv_stuffs=False #对面交易取出 则停止接收  交易完成/取消 则停止接收
      trade_finished=False  #交易状态 每次交易只会是单独的      
      #线程同步交易状态
      while True:
         stuffs = self.WaitRecvTradeStuffs(1)
         if stuffs:
            is_recv_stuffs=True#
            if stuffs.type == TRADE_STUFFS_ITEM:
               recv_stuffs["items"] = stuffs.items
            elif stuffs.type == TRADE_STUFFS_PET:              
               recv_stuffs["pets"][stuffs.pet.index] = stuffs.pet
            elif stuffs.type == TRADE_STUFFS_PETSKILL:
               recv_stuffs["petskills"][stuffs.pet.index]= stuffs.petskills
            elif stuffs.type == TRADE_STUFFS_GOLD:
               recv_stuffs["gold"]=stuffs.gold
         elif trade_finished:
            if is_recv_stuffs:
               return recv_stuffs
            else:
               return None
      return None
   def WaitTradeState(self):
      while True:
         trade_state =  self.WaitRecvTradeState(1)
         if trade_state:
            pass
   ...
   #延时执行函数 func执行的函数，delay延迟时间毫秒，args函数参数,kwargs函数参数
   #返回线程对象，可调用cancel函数来实现取消
   def setTimeout(self,func, delay, args=None, kwargs=None):
      t = threading.Timer(delay * 0.001, func,args,kwargs)
      t.start()
      return t
...
def 等待服务器返回(timeout):
   return cg.WaitRecvNpcDialog(timeout/1000)
def 等待聊天信息返回(timeout):
   return cg.WaitRecvChatMsg(timeout/1000)
def 等待工作返回(timeout):
   return cg.WaitRecvWorkingResult(timeout/1000)
def 等待交易对话框(timeout):
   return cg.WaitRecvTradeDialog(timeout/1000)
def 等待交易信息返回(timeout):
   return cg.WaitRecvTradeStuffs(timeout/1000)
def 等待交易状态返回(timeout):
   return cg.WaitRecvTradeState(timeout/1000)
def 等待菜单返回(timeout):
   return cg.WaitRecvPlayerMenu(timeout/1000)
def 等待菜单项返回(timeout):
   return cg.WaitRecvUnitMenu(timeout/1000)
def 等待战斗返回(timeout):
   return cg.WaitRecvBattleAction(timeout/1000)
def 等待连接状态返回(timeout):
   return cg.WaitRecvConnectionState(timeout/1000)
def 等待窗口按键返回(timeout):
   return cg.WaitRecvGameWndKeyDown(timeout/1000)
def 等待下载地图返回(timeout):
   return cg.WaitRecvDownloadMap(timeout/1000)
def 等待窗口关闭返回(timeout):
   return cg.WaitRecvServerShutdown(timeout/1000)
...
def 设置(*args):
   if len(args)==2:
      if args[0] == "移动速度":
         ...
         cfg_settings={"battle":{"autobattle":args[1]}}
         rsp = requests.post("http://127.0.0.1:"+cg.g_gui_port+'/cga/LoadSettings',json.dumps(cfg_settings) ,json=True)
         if rsp.status_code == 200:
            return True
      elif args[0] == "高速战斗":
         cfg_settings={"battle":{"highspeed":args[1]}}
         rsp = requests.post("http://127.0.0.1:"+cg.g_gui_port+'/cga/LoadSettings',json.dumps(cfg_settings) ,json=True)
         if rsp.status_code == 200:
            return True
      elif args[0] == "高速延时":
         cfg_settings={"battle":{"delayfrom":args[1]}}
         rsp = requests.post("http://127.0.0.1:"+cg.g_gui_port+'/cga/LoadSettings',json.dumps(cfg_settings) ,json=True)
         if rsp.status_code == 200:
            return True
      # elif args[0] == "遇敌全跑":
...
def 人物动作(*args):
   if len(args)==1:
      if args[0] == "换片":
         cg.DoCharacterAction(TCharacter_Action_EXCAHNGECARD)
交易 = cg.LaunchTrade
#艾岛定居=
def 加载脚本(module_path):
   path = Path(module_path)#提取系统路径 获得文件名称
   filename = path.name
   spec = importlib.util.spec_from_file_location(filename,module_path)#加载模块
   module = importlib.util.module_from_spec(spec)     #创建模块
   spec.loader.exec_module(module)     #执行模块
   return module           #返回模块
技能=cg.FindPlayerSkill
里堡加血=cg.SupplyCastle
工作 = cg.Work
卖堆叠物=cg.SellPileDir
城堡卖堆叠物=cg.SellCastlePile
扔叠加物=cg.ThrowNotFullItemName