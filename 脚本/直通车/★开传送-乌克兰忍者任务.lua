--定居艾尔莎岛 请注意,要将伊尔,圣村,维村传送开好,身上没有物品情况下，会重置任务从头开始，本脚本不会打忍者，只是开小道传送
...
::begin::
	...
	if(mapNum == 1000)then
		goto map1000
	elseif(mapNum == 1170)then
		goto map1170
	elseif(mapNum == 1500)then
		goto map1500	
	elseif(mapNum == 2100)then
		goto map2100
	elseif(mapNum == 59520)then
		goto map59520
	end
	goto begin
::map1000::
...
::map1170::
	...
	转向(2)
	dlg=等待服务器返回()	
--"\n\n搜查好像已经结束了。听说犯人是那个少女？唉···我还以为你多厉害呢！真是失望···"	
--"\n\n\n　　（※）结束搜查。\n\n　　　　　　　辛苦你了。"
...
::map2213::
	...
	喊话("毒...",2,3,1)
	对话选是(11,11)
	if(取物品数量("岚的念珠") > 0 or 取物品数量("岚的药丸") > 0 or 取物品数量("岚的戒指") > 0 or 取物品数量("岚的耳饰") > 0 or 取物品数量("岚的护手") > 0 or 取物品数量("岚的传承") > 0 or 取物品数量("心头灭却") > 0)then
		日志("心头灭却")
		回城()
