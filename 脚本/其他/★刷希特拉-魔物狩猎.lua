	...
	if(是否临时队长)then		
		...
		if(syncTargetTbl.东 ==0 and syncTargetTbl.南 == 0)then
			...
			if(是否目标附近(syncTargetTbl.东,syncTargetTbl.南,10) == false)then				
				日志("自动寻路到队长附近 东："..syncTargetTbl.东.." 南："..syncTargetTbl.南)
				自动寻路( tonumber(syncTargetTbl.东),tonumber(syncTargetTbl.南))				
			end	
