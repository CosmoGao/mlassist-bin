...
function 取银行宠物数量(petName)
	local count=0
	local bankData = 银行("宠物")
	for i,item in ipairs(bankData) do
...
function 取宠物数量(petName)
	local count=0
	local petData = 全部宠物信息()
	for i,pet in ipairs(petData) do
		if( pet.realname == petName)then
			count=count+1
		end
	end
	return count
...
::卖货::	
	...
	日志("改烈风45W一个，加队自己交易！",1)
	if(取宠物数量(售卖宠物名字) < 1)then goto 取货 end		
	...
	转向(6)
	等待交易("","宠物:改造烈风哥布林|1","金币:450000",10000)	
	等待(2000)
	goto 卖货  
::取货::
	if(取当前地图名() ~=  "银行")then 
	...
	等待服务器返回()
	银行("取宠",售卖宠物名字)
	if(取宠物数量(售卖宠物名字)  > 0)then goto 卖货 end		
	if(取银行宠物数量(售卖宠物名字) < 1)then
		goto 等上货喊话
	end
	goto 取货
::等上货喊话::
