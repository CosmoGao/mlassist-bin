require('./common').then(cga=>{
	...
	var takeOff = ()=>{
		...
		console.log("takeOff"+playerinfo.hp + "max hp" + playerinfo.maxhp  + "mp" + playerinfo.mp );
		if (playerinfo.hp == playerinfo.maxhp) {
			...
			console.log("takeOff delay"+playerinfo.hp + "max hp" + playerinfo.maxhp  + "mp" + playerinfo.mp );
            leo.delay(2000);
			console.log("takeOff delay End"+playerinfo.hp + "max hp" + playerinfo.maxhp  + "mp" + playerinfo.mp );
			if (playerinfo.hp == playerinfo.maxhp) {
			//穿上装备
			console.log("Put"+playerinfo.hp + "max hp" + playerinfo.maxhp  + "mp" + playerinfo.mp );
			...
			console.log("PutFini"+playerinfo.hp + "max hp" + playerinfo.maxhp  + "mp" + playerinfo.mp );
					...
					if(cga.GetPlayerInfo().mp < minMp){
						...
						.then(()=>{
							...
							if(skillLevel <= skill.lv){
						        leo.log('护士烧急救脚本，提示：技能【'+skillName+'】等级已达到【'+skill.lv+'】，达到或者超过了预设的目标等级【'+skillLevel+'】，脚本结束');
						        return leo.reject();
						    }else{
						        return leo.log('护士烧急救脚本，要烧的技能是【'+skillName+'】，目前等级【'+skill.lv+'】，预设的目标等级【'+skillLevel+'】');
						    }
						});
					}
				})
				.then(()=>{
					 console.log("input use skill");
					const skill = cga.GetSkillsInfo().find(s => s.name == '急救');
					const requireMp = 50;
					playerinfo = cga.GetPlayerInfo();
					 console.log("input use skill"+playerinfo.hp + "max hp" + playerinfo.maxhp + skill + playerinfo.mp );
